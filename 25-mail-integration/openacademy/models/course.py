...
class Course(models.Model):
    ...
    def _add_follower(self, vals):
        if vals.get('responsible_id'):
            related_partner = self.env['res.users'].browse(vals.get('responsible_id')).partner_id
    ...
    def message_get_suggested_recipients(self):
        ...
        for session in self.session_ids:
            result[self.ids[0]].append((session.instructor_id.id, '%s<%s>' % (session.instructor_id.name, session.instructor_id.email), 'Session Instructor'))
        return result
